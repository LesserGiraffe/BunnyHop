...
public class RemoteCommunicator {
	...
	BhRemoteServer rcp = null;	//!< RMIオブジェクト
	...
	public void sendFile(Path filePath, Consumer<Boolean> onTransferEnd, String ipAddr) {
		...
		Runnable sendFunc = () -> {
			...
			for (int i = 0; i < BhParams.ExternalProgram.numFindingRmiObjRetry+1; ++i) {
	 ...
	 */
	private synchronized void findRCP(boolean doAgain, String ipAddr) throws MalformedURLException, NotBoundException, RemoteException {
		
		if ((!doAgain) && (rcp != null)) {	//探す必要なし
			return;
		}
		rcp = (BhRemoteServer)Naming.lookup("rmi://" + ipAddr + ":" + BhParams.ExternalProgram.bhServerTCPPort + "/RemoteCmdProcessor");
	}
}
