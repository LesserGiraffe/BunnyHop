...
//		}
	...
	private void highlightOverlappedNode() {
		...
		ArrayList<BhNode> overlappedList = view.getViewRegionManager().searchOverlappedModel();	//このノードとコネクタ部分が重なっている
	...
	public MsgData receiveMsg(BhMsg msg, MsgData data) {
		...
		switch (msg) {
			...
			case REMOVE_QT_RECTANGLE:
				view.getViewRegionManager().removeQtRectable();
			...
			case GET_VIEW_SIZE_WITH_OUTER:
				Point2D size = view.getPositionManager().getBodyAndOuterSize(data.bool);
			...
			case REPLACE_NODE_VIEW:	//このコントローラの管理するノードビューを引数のノードビューと入れ替える (古いノードのGUIツリーからの削除は行わない)
				...
				view.getViewTreeManager().replace(newView);	//新しいノードビューに入れ替え
			...
			case REMOVE_FROM_GUI_TREE:
				view.getViewTreeManager().removeFromGUITree();
				break;
				
			default:
				MsgPrinter.instance.MsgForDebug("BhNodeController.receiveMsg errorMsg");
				assert false;
		}
